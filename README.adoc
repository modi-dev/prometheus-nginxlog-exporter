...
When running in Kubernetes, you should set a memory limit on the exporter
container. The exporter can automatically adapt Go's heap target to the
container cgroup memory limit by using:

- `-auto-gomemlimit=true` (default)
- `-gomemlimit-ratio=0.9` (default)

With these defaults, the exporter configures `GOMEMLIMIT` to 90% of the
container memory limit (unless `GOMEMLIMIT` is already set explicitly).
